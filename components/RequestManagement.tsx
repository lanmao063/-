...
const RequestManagement: React.FC = () => {
  ...
  const getRequestTypeInfo = (type: RequestType) => {
    switch (type) {
      ...
      case RequestType.REBALANCING: return { label: lang === 'zh' ? 'AI 系统调仓建议' : 'AI Rebalance Suggestion', color: 'text-purple-600', bg: 'bg-purple-50', isAI: true, isNotice: false };
  ...
  const getAIReason = (id: string) => {
    if (id.includes('001')) return lang === 'zh' ? '科技股持仓已触及 70% 止盈线，建议回调至 60%。' : 'Tech exposure hit 70% take-profit line. Recommend shift to 60%.';
    if (id.includes('002')) return lang === 'zh' ? '识别到 ESG 评级调级，建议移除某不合规标的并增持绿色能源。' : 'ESG rating downgraded. Recommend removal of non-compliant assets.';
    if (id.includes('004')) return lang === 'zh' ? '国债收益率预期变动，建议增加长久期债权配置。' : 'Bond yield expectations shifted. Recommend increasing long-duration bonds.';
    return lang === 'zh' ? '检测到市场波动性上升，建议对冲性仓位调整。' : 'Market volatility rising. Hedging adjustment recommended.';
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
      <div className="lg:col-span-2 space-y-6 flex flex-col">
        <div className="flex items-center justify-between shrink-0">
          <h1 className="text-2xl font-bold text-slate-800">{lang === 'zh' ? '审批任务中心' : 'Audit Task Center'}</h1>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {info.isAI && <span className="text-[9px] font-black uppercase tracking-widest bg-purple-600 text-white px-1.5 py-0.5 rounded">AUTO</span>}
                    ...
                    <div className="text-xs text-slate-500 mt-0.5">
                      {req.customerName} | {req.date}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-slate-800">
                      {req.amount > 0 ? `¥${req.amount.toLocaleString()}` : (lang === 'zh' ? '配置调整' : 'Realloc')}
                    </div>
                    <div className={`flex items-center justify-end gap-1.5 mt-1 text-[10px] font-bold uppercase ${
                      ...
                      {req.status === RequestStatus.SUCCESS ? (lang === 'zh' ? '已入档' : 'Archived') : 
                       req.status === RequestStatus.FAILED ? (lang === 'zh' ? '已否决' : 'Vetoed') : (lang === 'zh' ? '待处理' : 'Pending')}
                    </div>
                  </div>
                </div>
              );
        ...
        {selectedRequest ? (
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl p-6 sticky top-8 animate-in slide-in-from-right duration-300">
            <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center justify-between">
              {viewMode === 'ACTIVE' ? (lang === 'zh' ? '任务处理' : 'Processing') : (lang === 'zh' ? '档案详情' : 'Archive')}
              ...
              {selectedRequest.type === RequestType.REBALANCING ? (
                <div className="space-y-4">
                  <div className={`p-4 rounded-lg border ${viewMode === 'ACTIVE' ? 'bg-purple-50 border-purple-100' : 'bg-slate-50 border-slate-200'}`}>
                    <p className="text-[10px] font-bold text-purple-900 mb-2 uppercase tracking-widest">{lang === 'zh' ? 'AI 推理摘要' : 'AI Reasoning'}</p>
                    <p className="text-xs text-slate-600 leading-relaxed italic">"{getAIReason(selectedRequest.id)}"</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-lg space-y-3">
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span className="text-slate-500">{lang === 'zh' ? '组合' : 'Portfolio'}</span>
              ...
              {viewMode === 'ACTIVE' && (
                <div className="pt-6 border-t border-slate-100 space-y-4">
                  {selectedRequest.type === RequestType.TRANSFER_OUT ? (
                    ...
                    /* 调仓建议显示 Approve/Reject */
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                        <MessageSquare className="w-3.5 h-3.5 text-slate-400" />
                        <input type="text" placeholder={lang === 'zh' ? '备注审批意见...' : 'Add notes...'} className="flex-1 bg-transparent text-xs outline-none" />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <button className="py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition-colors">
                          {lang === 'zh' ? '驳回建议' : 'Veto'}
                        </button>
                        <button className="py-2.5 rounded-xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 transition-all shadow-lg">
                          {lang === 'zh' ? '执行调仓' : 'Execute'}
                        </button>
                      </div>
                    </div>
                  )}
              ...
              {viewMode === 'HISTORY' && (
                <div className="pt-4 border-t border-slate-100 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{lang === 'zh' ? '归档时间' : 'Archived At'}</span>
                    <span className="text-xs font-medium text-slate-600">{selectedRequest.date} 15:45</span>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 mb-1 uppercase">{lang === 'zh' ? '处理摘要' : 'Audit Summary'}</p>
                    <p className="text-xs text-slate-600">
                      {selectedRequest.status === RequestStatus.SUCCESS 
                        ? (lang === 'zh' ? '任务已成功同步至券商交易接口。' : 'Task successfully synced to brokerage.') 
                        : (lang === 'zh' ? '因[经理干预]驳回：人工判断市场流动性不足。' : 'Vetoed by manager: Liquidity assessment.')}
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="h-[400px] border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-400 p-8 text-center">
             ...
             <p className="text-sm font-bold text-slate-600">{lang === 'zh' ? '请选择审批项目' : 'Select an Item'}</p>
             <p className="text-[10px] mt-2">{lang === 'zh' ? '点击左侧列表查看详细的 AI 建议或业务通知' : 'Click list items to see AI insights or notices'}</p>
          </div>
        )}
      </div>
    </div>
